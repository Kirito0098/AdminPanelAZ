...
> Финальный релизный этап плана. Текущая панель в репо — смотри badge/CHANGELOG (сейчас **2.18.0**); новая версия — следующий semver (скорее **2.19.0**, если нет breaking API).
>
> Обязательно:
...
- Перенеси накопленное из [Unreleased] в новую секцию ## [X.Y.Z] - YYYY-MM-DD
  (X.Y.Z = следующий minor после последней релизой, обычно 2.19.0; дата = день релиза).
