...
- **Панель открывалась по IP сервера за Nginx** — установка vhost панели удаляла `sites-enabled/default`, и сервером по умолчанию для голого IP и чужих имён становился vhost панели: `http://IP` редиректил на `https://IP`, а после предупреждения браузера открывалась панель. Теперь вместе с vhost ставится `sites-available/00-adminpanelaz-default-deny`: на HTTP-порту `return 444`, на HTTPS — `ssl_reject_handshake on` (nginx ≥ 1.19.4) или свой самоподписанный сертификат и `444` на старом nginx. Порт с чужим `default_server` не трогается; сбой `nginx -t` откатывает только этот файл, публикация панели не падает. При переходе на прямую публикацию файл убирается (uvicorn сам занимает HTTPS-порт), `uninstall.sh` его удаляет. **Чтобы закрыть доступ по IP на уже опубликованной панели, перегенерируйте nginx** (`scripts/nginx-repair.sh` или переключение proxy-mode в настройках).
...
- Shell: `test-nginx-default-deny.sh` — сервер по умолчанию ставится на порты vhost панели, повтор не считает свой файл чужим, вариант для nginx 1.18 с сертификатом (ключ `600`), чужой `default_server` на порту пропускается (`80` не путается с `8080`/`18080`, закомментированные и выключенные сайты не считаются), сбой `nginx -t` откатывает только default-deny, переход на прямую публикацию убирает файл и перезагружает nginx уже без него; при наличии nginx — живая проверка: голый IP по HTTP получает закрытое соединение, по HTTPS — отказ в рукопожатии, домен панели отвечает (добавлен в CI).
