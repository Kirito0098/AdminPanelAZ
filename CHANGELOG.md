...
- **Прокси-узлы (волна 1)** — feature toggle `proxy_nodes` / `FEATURE_PROXY_NODES_ENABLED` (**default off**): узлы `node_kind=proxy`, отдельный `proxy_agent` на RU (`:9101`, systemd `adminpanelaz-proxy`), панель через `ProxyNodeAdapter` — health/status, смена DESTINATION в iptables, mappings. Панель **не** ставит `proxy.sh`. Прокси нельзя активировать как VPN. Docs: [uzly.md](docs/uzly.md), [proxy-agent.md](docs/proxy-agent.md).
- **Прокси-узлы (волна 2, NOC IP)** — при включённом модуле overview NOC склеивает сессию с mappings `proxy_agent`: домашний IP + geo при совпадении порта, иначе IP прокси + пометка «через прокси» / «IP не восстановлен» в списке подключений. Docs: [noc-monitoring.md](docs/noc-monitoring.md#подключения-через-прокси-узел).
