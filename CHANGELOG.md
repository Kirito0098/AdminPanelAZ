...
- **Атомарная запись `.env`** — `.env` панели (настройки Cloudflare, публикации, ротация секретов и остальные сохранения через настройки), `node_agent.env` при включении mTLS и при ротации ключа агента перезаписывались на месте: сбой или нехватка места посреди записи оставляли обрезанный файл, и панель или агент не поднимались. Теперь запись идёт во временный файл в том же каталоге (`600`), с `fsync`, `rename` и `fsync` каталога; при ошибке прежний файл остаётся нетронутым. Записанный файл всегда `600`, symlink на `.env` сохраняется.
