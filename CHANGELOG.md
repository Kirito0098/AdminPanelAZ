...
- Backend: `test_refresh_token_rotation` — повтор заменённого токена отзывает цепочку (и все токены пользователя для записей без `family_id`), окно 30 с для параллельных вкладок, проигравший гонку запрос не получает новую cookie, logout/истечение/отключённый пользователь, миграция колонок и индекса.
