...
- **Клиентский портал** — постоянные шаринг-ссылки на админ-заданном поддомене (`portal_domain` в **Выдача VPN-профилей**): страница `/p/{token}` с установкой, для OpenVPN — `openvpn://import-profile/…` и скачивание `.ovpn`, для WG/AWG — скачивание конфига; create/rotate/revoke в карточке клиента. One-time QR при заданном хосте портала тоже строятся с него. Модуль `client_portal`.
