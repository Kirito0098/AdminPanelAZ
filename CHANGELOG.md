...
> **Кратко:** мониторинг трафика — корректная доля в таблице, TB для больших объёмов, без переноса «GB»; мониторинг сервера — исправлен live CPU, графики ресурсов, понятные названия интерфейсов, улучшен hover на графиках; ускорена загрузка overview и убран дублирующий запрос.
...
- **Ежедневная/еженедельная сводка в TG** — трафик за период с Δ к прошлому периоду; ресурсы CPU/RAM/Диск (среднее и пик); lag сбора трафика; блокировки по лимиту; сессии OVPN/WG (среднее и пик); топ клиентов; алерты; CIDR; офлайн-узлы (`noc_report.py`, `resource_metrics.py`).
- **Предпросмотр NOC в настройках Telegram** — кнопки «Ежедневная сводка» / «Еженедельная сводка» / «Еженедельный PDF»; `POST /settings/admin-notify/test-noc-report`, `test-noc-pdf`.
- **Тест каждого типа TG-уведомления** — кнопка отправки примера у каждого переключателя «О чём сообщать»; `POST /settings/admin-notify/test-event`; исправлено формирование текста для `cidr_ingest_partial`.
