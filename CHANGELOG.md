...
- **Узлы / сессия** — неверный API-ключ remote-узла больше не отдаётся как HTTP 401 и не выбивает админа из панели; refresh cookie крутится через один mutex (`silentRefresh` + `apiFetch`).
