...
- **Node.js** — установщик требует Node.js 22+ и ставит 24.x (Active LTS) вместо 20.x, который больше не поддерживается; версия из apt принимается по мажорной версии кандидата. `frontend/package.json` объявляет `engines.node >=22`, CI проверяет фронтенд на Node 22 и 24. На установленных панелях Node нужно обновить отдельно (повторный `install.sh` или NodeSource).
...
- **Меню / обновление** — `adminpanel-menu.sh --update` пересобирает интерфейс (`npm install`, `npm run build:all`), как обновление из панели, если интерфейс на сервере уже собирали; при сбое сборки или без npm завершается ошибкой с командами для ручной пересборки. Раньше после обновления из меню работал старый интерфейс.
