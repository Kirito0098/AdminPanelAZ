...
- **Подписки / портал пользователей** — кнопки «Перевыпустить» и «Отозвать» в разделе «Портал пользователей» сразу отзывали все ссылки пользователя, одно случайное нажатие ломало ссылку, которую он уже получил. Теперь обе кнопки сначала спрашивают подтверждение с именем пользователя и предупреждением, что старая ссылка перестанет открываться.
- **OpenVPN Buffer Guard / ввод чисел** — числовые поля (порог, окно, задержка эскалации, пауза, временный бан) зажимались в пределы на каждое нажатие: набрать «150» в пороге было нельзя — после первой «1» поле сразу становилось «10», а очищенное поле тут же заполнялось минимумом. Теперь число вводится свободно, под полем показывается допустимый диапазон, если набранное в него не попадает, а в пределы значение приводится, когда поле теряет фокус, и при сохранении; пустое поле возвращает прежнее значение.
- **AZ-WARP / данные прежнего узла после переключения** — если узел переключали, пока раздел AZ-WARP загружался, поздний ответ по прежнему узлу показывался как данные нового: режим и настройки, домены и подсети (а «Сохранить» после этого записывал их на новый узел как «сохранённые»), каталог, логи, трафик, диагностика, проверка обновлений и статус автопоиска. Теперь все разделы применяют только ответ на последний запрос для показываемого узла, при смене узла сбрасывают тексты доменов и подсетей, а страница не подставляет во вкладку «Домены» список, загруженный для прежнего узла.
- **Узлы / смена узла после сбоя опроса** — если опрос активного узла один раз не удавался (обрыв сети, перезапуск панели), вкладка забывала, какой узел показывает, и следующий опрос молча переключал её на узел, выбранный в другом месте, без предупреждения. Теперь неудачный опрос ничего не меняет: вкладка остаётся на своём узле, а смена узла по-прежнему показывается предупреждением.
- **Вход / «Панель недоступна» вместо входа** — если у вошедшего пользователя фоновая проверка профиля (после смены темы, часового пояса, привязки Telegram) не достучалась до сервера, панель запоминала «сервер недоступен». Когда сессия потом заканчивалась, вместо страницы входа показывался экран «Панель недоступна», хотя сервер уже ответил. Теперь сбой связи запоминается только для вкладки без вошедшего пользователя, а завершение сессии сбрасывает его.
- **OpenVPN Buffer Guard / настройки не того узла** — если узел переключали, пока карточка Buffer Guard загружала настройки, поздний ответ по прежнему узлу становился черновиком нового, и «Сохранить» записывал пороги и режим одного узла на другой. Теперь ответы по узлу, который карточка уже не показывает, и ответы на устаревшие запросы отбрасываются, черновик сбрасывается при смене узла, а сохранение идёт только на узел, для которого черновик загружен.
- **AZ-WARP / переключатели FullVPN и «Автопатч DNS»** — если сохранение не удавалось, переключатель оставался в новом положении, хотя на узле ничего не изменилось. Теперь при ошибке он возвращается в прежнее положение и показывается ошибка.
- **Узлы / удаление активного узла** — после удаления активного узла вкладка, из которой его удалили, показывала «Активный узел сменён в другом месте», каждое изменение из неё получало `409`, а кнопка «Вернуть» отвечала `404`: сервер делал активным локальный узел, а вкладка продолжала называть в запросах удалённый. Теперь вкладка без предупреждения переходит на новый активный узел (и при пакетном удалении), удалённый узел сразу пропадает из списка, а «Вернуть» не предлагается для узла, которого больше нет.
- **Обновление из панели / `npm install` вместо `npm ci`** — обновление из раздела «Обновления» ставило зависимости интерфейса через `npm install`, который не обязан держаться `package-lock.json`: может поставить другие версии и изменить lock-файл в рабочем каталоге (тогда следующий `git pull` спотыкается о локальные изменения). Установщик и меню уже использовали `npm ci`. Теперь и обновление из панели выполняет `npm ci`: ровно версии из `package-lock.json`; сбой шага показывается как «Ошибка: npm ci», сборка и перезапуск не выполняются.
- **Nginx / geo Cloudflare в `conf.d` без отката** — `conf.d/adminpanelaz-cloudflare-origin.conf` подключается nginx глобально, но при установке сайта перезаписывался без отката: если `nginx -t` не проходил, откатывались vhost и `server_names_hash`, а новый geo оставался (копия была только в `/etc/nginx/backups`). Теперь при неудачном `nginx -t` (выделенный vhost панели, портал, установщик, режим subpath) geo возвращается к виду до установки, а созданный заново — удаляется. После успешной проверки временная копия удаляется, история в `/etc/nginx/backups` сохраняется как раньше.
- **Разработка / тест вебхука Telegram и ESLint** — `test-nginx-telegram-webhook-realip.sh` без `ENV_FILE` подключал библиотеку nginx к рабочему `backend/.env`. Проверки его пока не читали, но любая новая могла прочитать или изменить настройки сервера. Теперь тест работает с временным `.env`, а `test-host-guard.sh` проверяет это для всех тестов с `nginx-common.sh`. `npx eslint .` проверял `frontend/dist.next` и `frontend/dist.old` (промежуточные сборки `npm run build`, остающиеся после прерванной сборки) — собранный код, который линтить не нужно. Теперь они игнорируются, как `dist`.
- **Меню / первое обновление с 2.25.1 не пересобирало интерфейс** — bash выполняет версию `adminpanel-menu.sh`, прочитанную до `git pull`, поэтому обновление с 2.25.1 через меню шло по старому коду: pip и systemd units, без сборки интерфейса. То же ждало бы любые новые шаги обновления в будущих версиях. Теперь после `git pull` шаги обновления (pip, сборка интерфейса, units) выполняет уже обновлённый скрипт с диска. `--update` без новых коммитов пересобирает интерфейс, если какой-либо файл `frontend` из git новее собранного `frontend/dist/index.html`; `--restart` в этом случае предупреждает. **Обновляетесь с 2.25.1 через меню:** старое меню обновит только код. После него выйдите из меню и выполните `sudo ./scripts/adminpanel-menu.sh --update` ещё раз: новое меню пересоберёт интерфейс. Затем `sudo ./scripts/adminpanel-menu.sh --restart`. Вручную: `cd frontend && npm ci && npm run build:all`.
...
- `userPortalConfirm.test.ts` — подтверждение перевыпуска и отзыва ссылки портала называет пользователя и предупреждает, что старая ссылка перестанет открываться; при перевыпуске — что пользователю нужно отправить новую.
- `bufferGuardDraft.test.ts` (числовые поля) — промежуточный ввод не считается ошибкой, пока попадает в диапазон; значение вне пределов приводится к ближайшей границе, дробное округляется, пустое или нечитаемое оставляет прежнее; при сохранении учитываются поля, которые ещё редактируются.
- `latestRequest.test.ts` (`runLatest`) — ответ или ошибка применяются и снимают флаг загрузки только у последнего запроса для показываемого узла; поздний ответ и поздняя ошибка по прежнему узлу не трогают ни данные, ни флаг загрузки; из двух пересекающихся запросов результат применяет только новый.
- `authBoot.test.ts` — недоступный сервер показывается только вкладке без пользователя; после неудачной фоновой проверки и завершения сессии показывается вход, а не «Панель недоступна»; старое «недоступна» забывается при завершении сессии и при ответе сервера.
- `latestRequest.test.ts`, `bufferGuardDraft.test.ts` — ответ применяется только к последнему запросу для показываемого узла; ответы по прежнему узлу и запросы, начатые до смены узла, отбрасываются; запрос по узлу, который уже не показывается, не вытесняет текущий. Черновик Buffer Guard привязан к узлу, для которого загружен, и не сохраняется на другой.
- `warper/utils.test.ts` — переключатель AZ-WARP показывает новое положение на время сохранения, оставляет его после успеха и возвращает прежнее, если сохранение не удалось или бросило ошибку.
- `expectedNode.test.ts` — неудачный опрос активного узла не снимает защиту: следующий опрос с другим узлом показывает предупреждение, а не переключает вкладку; опрос, пересёкшийся с выбором узла в этой вкладке, отбрасывается.
- `expectedNode.test.ts` — удаление узла, который показывает вкладка, переводит её на запасной узел без предупреждения; ответы опроса, пришедшие во время удаления, отбрасываются; если запасной узел неизвестен, вкладка не называет узел в записях; удаление другого узла и неудачное удаление защиту от молчаливой смены не снимают; «Вернуть» предлагается только для существующего узла.
- `test_system_update_npm_ci.py` — обновление из панели ставит зависимости интерфейса через `npm ci` в `frontend`; сбой `npm ci` останавливает обновление до сборки и перезапуска и попадает в ошибки и вывод.
- `test-nginx-cloudflare-geo-rollback.sh` — неудачный `nginx -t` при установке выделенного vhost, портала (geo пишется в подоболочке рендера) и в режиме subpath возвращает geo Cloudflare к прежнему виду или удаляет созданный; актуальный geo не трогается; двойное изменение откатывается к состоянию до установки; после успеха в `conf.d` не остаётся временных копий, прежний geo лежит в backups.
- `test-host-guard.sh` — каждый тест, подключающий `nginx-common.sh`, до подключения задаёт `ENV_FILE`, не указывающий на рабочий `backend/.env`.
- `eslintIgnores.test.ts` — ESLint игнорирует `dist`, `dist.next` и `dist.old` и по-прежнему проверяет `src`.
- `test-menu-update-rebuilds-frontend.sh` — после `git pull` шаги выполняет скрипт меню из обновлённого репозитория, его код возврата сохраняется; при актуальном репозитории интерфейс пересобирается, если файл `frontend` из git новее `dist/index.html` или `index.html` нет, и не пересобирается из-за файлов вне git и на сервере без `frontend/dist`; сбой такой сборки даёт ошибку с командой для ручной сборки; `--restart` предупреждает об устаревшем интерфейсе, но сам не собирает.
