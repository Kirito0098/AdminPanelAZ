...
- Backend: `test_worker_leader` — блокировка лидера исключительна между воркерами (права `600`); лидер запускает фоновые задачи и одноразовые действия старта, остальные воркеры не запускают ничего и подхватывают задачи (без одноразовых действий) после выхода лидера; lifespan освобождает блокировку при остановке. `test_server_reboot` — отмена и список из другого воркера, дубль между воркерами, время в UTC, перезагрузка завершившегося воркера или просроченная не блокирует новую и не показывается в списке (список ничего не пишет), долгая перезагрузка работающего воркера сохраняет статус, при старте прерываются только перезагрузки завершившихся воркеров, ошибки таймера пишутся в журнал. `test_process_identity` — владелец отличает живой процесс от завершившегося и от процесса с повторно выданным PID. `test_background_task_recovery` — при старте прерываются только задачи завершившихся воркеров.
