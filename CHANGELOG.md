...
- Backend: `test_backup_manager` — не-SQLite, обрезанная и не прошедшая `integrity_check` БД в архиве отклоняются без изменения рабочей базы; копия `.pre-restore` совпадает с прежними файлами (права `700`/`600`, хранится `PRE_RESTORE_KEEP`); восстановленные файлы `600`, старый WAL удалён; сбой записи `.env` возвращает уже заменённую БД, временные файлы не остаются.
- Backend: `test_db_migrations_atomic` — сбой посреди пересоздания таблицы не оставляет `_new` и повтор проходит; остаток `_new` от старого сбоя не мешает; перенос WireGuard доводится без повторного пересоздания OpenVPN (привязка к узлу сохраняется); DDL откатывается; миграции идут под эксклюзивной блокировкой (`600`).
- Backend: `test_traffic_session_retention` — чистка удаляет только завершённые сессии старше настроенного срока (активные остаются) и refresh-токены, истёкшие больше 7 дней назад; сборщик и пересчёт базы сессий загружают только активные сессии и ключи из снимка, вернувшаяся сессия переиспользуется и за пределами первого пакета ключей (1200 клиентов); `PATCH /settings/retention` сохраняет `TRAFFIC_SESSION_RETENTION_DAYS`.
