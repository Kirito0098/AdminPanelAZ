...
- Backend: `test_async_endpoints_offload` — потоки мониторинга, установки AZ-AWG2 и обновления AZ-WARP, WebSocket монитора сервера, восстановление AZ-AWG2, загрузка бэкапа и импорт CSV выполняют блокирующую работу вне потока event loop; `iterate_in_thread` закрывает исходный поток при раннем отключении.
- Backend: `test_telegram_bot_loop` — блокирующий обработчик бота не останавливает event loop панели, обновления идут на одном loop бота, ошибки доходят до вебхука, начатое действие не прерывается отменой запроса, вебхук обрабатывает обновление в потоке бота.
