...
- Backend: `test_worker_leader` — блокировка лидера исключительна между воркерами (права `600`); лидер запускает фоновые задачи и одноразовые действия старта, остальные воркеры не запускают ничего и подхватывают задачи (без одноразовых действий) после выхода лидера; lifespan освобождает блокировку при остановке. `test_server_reboot` — отмена и список из другого воркера, дубль между воркерами, время в UTC, брошенная перезагрузка не блокирует новую, прерывание при старте панели.
