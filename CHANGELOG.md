...
- **Мультиконфиг при создании профиля** — в «Новый клиент» можно выбрать несколько протоколов (OpenVPN / WG / AWG2) под одним именем; диалог с прокруткой и закреплённым футером, чтобы не уезжал за экран. Адаптивная ширина/высота: телефон → MacBook 13–16 (`lg`/`xl` по `vw`) → большие мониторы с потолком; на низкой высоте учитываются Dock/меню. Вместо one-click шаблонов — ручной **лимит трафика** на все выбранные конфигурации: без сброса / 1 день / 7 дней / месяц.
