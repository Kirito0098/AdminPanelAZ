...
- **HA / блокировки WireGuard** — синхронизация крипто-состояния с primary (создание/удаление клиента, heal дрейфа) и Push full снимали на репликах ручные и временные блокировки WireGuard: `wg syncconf` из конфигов primary возвращал удалённых из runtime peer'ов. Теперь после синхронизации блокировки переприменяются на реплике; Push full делает это после копирования политик доступа (для WireGuard и AWG2).
...
- Backend: `test_vpn_state_sync` / `test_node_sync_push_full` — блокировки WireGuard переприменяются на реплике после `syncconf`, heal и Push full (после копирования политик); ошибка переприменения помечает реплику как неуспешную.
