...
- **Telegram / webhook** — заголовок `X-Telegram-Bot-Api-Secret-Token` теперь обязателен (панель передаёт его при регистрации webhook с 1.8.0); `X-Real-IP` учитывается только от доверенного прокси (`TRUSTED_PROXY_IPS`), поэтому проверку IP Telegram нельзя обойти подделкой заголовка. **Если бот перестал отвечать после обновления — нажмите «Зарегистрировать webhook»** в настройках Telegram.
