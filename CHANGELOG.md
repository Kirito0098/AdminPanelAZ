...
> **Кратко:** NOC в Telegram — расширенные сводки, еженедельный PNG-дашборд вместо PDF, предпросмотр и тест каждого типа уведомления; все TG-алерты — единый карточный формат (заголовок → актор → детали → время), вход с IP и устройством; мониторинг трафика/сервера — TB, доля, live CPU, графики ресурсов.
