...
def test_push_full_continue_on_error_processes_all_replicas():
    ...
    copy_profiles.assert_any_call(primary_adapter, adapter_ok_1)
    copy_profiles.assert_any_call(primary_adapter, adapter_ok_2)


def test_push_full_copies_openvpn_profiles_after_restore():
    ...
    assert result["openvpn_profile_copy"] == [{"node_id": 2, "node_name": "replica-1", "success": True}]
