...
def test_should_start_traffic_collector_uses_settings_only(monkeypatch):
    ...
    monkeypatch.setattr(lifecycle, "get_settings", lambda: _On())
    assert lifecycle.should_start_traffic_collector() is True

    monkeypatch.setattr(lifecycle, "get_settings", lambda: _Off())
    assert lifecycle.should_start_traffic_collector() is False
