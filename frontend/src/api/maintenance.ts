...
import type {
  ...
  VpnNetworkPublishPayload,
  BackgroundTaskAcceptedResponse,
  DdnsSettings,
  DdnsSettingsUpdatePayload,
  DdnsActionResponse,
  CloudflareProxySettings,
  CloudflareProxySettingsUpdatePayload,
...
export async function uploadBackup(file: File, restore = false) {
  ...
  return apiFetch<BackupEntry>('/backups/upload', {
    method: 'POST',
    body: form,
  })
}
...
export async function getVpnNetworkSettings() {
  return apiFetch<VpnNetworkSettings>('/settings/vpn-network')
}

export async function getVpnNetworkDomainSsl(domain: string) {
  const params = new URLSearchParams({ domain })
  return apiFetch<VpnNetworkDomainSslStatus>(
    `/settings/vpn-network/domain-ssl?${params.toString()}`,
  )
}

export async function getVpnNetworkPortStatus(port: number, role: VpnNetworkPortRole = 'backend') {
  const params = new URLSearchParams({ port: String(port), role })
  return apiFetch<VpnNetworkPortStatus>(
    `/settings/vpn-network/port-status?${params.toString()}`,
  )
}

export async function publishVpnNetwork(data: VpnNetworkPublishPayload) {
  return apiFetch<BackgroundTaskAcceptedResponse>('/settings/vpn-network/publish', {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export async function getDdnsSettings() {
  return apiFetch<DdnsSettings>('/settings/vpn-network/ddns')
}

export async function updateDdnsSettings(data: DdnsSettingsUpdatePayload) {
  return apiFetch<DdnsActionResponse>('/settings/vpn-network/ddns', {
    method: 'PUT',
    body: JSON.stringify(data),
  })
}
...
export async function runDdnsUpdate() {
  return apiFetch<DdnsActionResponse>('/settings/vpn-network/ddns/update', {
    method: 'POST',
  })
}

export async function getCloudflareProxySettings() {
  return apiFetch<CloudflareProxySettings>('/settings/cloudflare-proxy')
}

export async function updateCloudflareProxySettings(
  data: CloudflareProxySettingsUpdatePayload,
) {
  return apiFetch<CloudflareProxySettings>('/settings/cloudflare-proxy', {
    method: 'PATCH',
    body: JSON.stringify(data),
...
export async function refreshCloudflareProxy(force = false) {
  return apiFetch<CloudflareProxyRefreshResponse>('/settings/cloudflare-proxy/refresh', {
    method: 'POST',
...
export async function registerTelegramWebhook() {
  return apiFetch<TelegramSettings>('/settings/telegram/webhook/register', {
    method: 'POST',
  })
}

export async function deleteTelegramWebhook() {
  return apiFetch<TelegramSettings>('/settings/telegram/webhook', {
    method: 'DELETE',
  })
}

export async function getTelegramLinkCode() {
  return apiFetch<TelegramLinkCode>('/telegram/link-code')
}

export async function getTelegramBotInfo() {
  return apiFetch<TelegramBotInfo>('/telegram/bot-info')
}
...
export async function getAdminNotifySettings() {
  return apiFetch<AdminNotifySettings>('/settings/admin-notify')
}
...
export async function analyzeDpiLog(dpiLogText: string) {
  return apiFetch<DpiAnalysisResult>('/routing/cidr-db/analyze-dpi', {
    method: 'POST',
...
export async function getAntizapretSettings() {
  return apiFetch<AntizapretSettingsResponse>('/routing/antizapret-settings')
}

export async function updateAntizapretSettings(updates: Record<string, string | boolean>) {
  return apiFetch<AntizapretSettingsUpdateResponse>('/routing/antizapret-settings', {
    method: 'PUT',
...
export async function getOpenVpnGroup() {
  return apiFetch<OpenVpnGroupState>('/configs/openvpn-group')
}
...
export async function applySystemUpdate() {
  return apiFetch<BackgroundTaskAcceptedResponse>('/system/update', { method: 'POST' })
}
...
export async function cancelServerReboot(rebootId: string) {
  return apiFetch<ServerRebootPendingItem>(
...
export async function getPendingServerReboots() {
  return apiFetch<ServerRebootPendingResponse>('/settings/reboot/pending')
}

export async function rebuildPanel() {
  return apiFetch<BackgroundTaskAcceptedResponse>('/system/rebuild', { method: 'POST' })
}

export async function getScannerBans() {
  return apiFetch<{ active_bans: ScannerBan[] }>('/security/scanner-bans')
}
