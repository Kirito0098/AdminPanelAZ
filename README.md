...
В UI на вкладке **Модули** показывается замер **только стека AdminPanelAZ**: панель + локальная нода и её VPN-сервисы (`ANTIZAPRET_PATH`). Сторонние проекты на том же VDS не входят в цифру.

| Сценарий | Стек AdminPanelAZ + локальная нода (типично) | VDS RAM |
|----------|---------------------------------------------|---------|
| Только панель (VPN на других узлах) | **~200–280 MB** | **1 GB** + swap |
| Панель + node agent + VPN на одном VDS | **~280–400 MB** | **2 GB+** |
| Профиль **Full** (больше collectors у панели) | +**50–80 MB** к Standard | **2 GB+** |

Профили задаются в мастере или в UI: **Настройки → Модули → Профили ресурсов**. После смены — перезапустите панель. Подробнее: [docs/nastrojki/moduli.md](docs/nastrojki/moduli.md).
