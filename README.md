...
| Панель + локальный AntiZapret | Панель и VPN на одном хосте |
| Только Node agent | Только агент на VPN-сервере (без панели) |
| Полный стек | Панель, AntiZapret и node agent на одном сервере |

#### 2. AntiZapret

Путь к каталогу AntiZapret (по умолчанию `/root/antizapret`). Установщик проверяет наличие `client.sh`. Без AntiZapret панель запустится, но VPN-функции будут ограничены.
...
Рекомендуется: backend только на localhost, наружу — через Nginx (шаг 5).

#### 3a. Динамический DNS (DDNS)
...
Каталоги состояния (`/var/lib/adminpanelaz` при systemd), бэкапы.
...
sudo ./scripts/uninstall.sh            # удалить systemd/nginx timer
```
...
Для публикации панели в интернете без покупки домена можно использовать бесплатный **динамический DNS (DDNS)** — поддомен, который указывает на ваш публичный IP. Установщик поддерживает **DuckDNS** и **No-IP**; остальные настраиваются вручную по инструкциям провайдера, затем домен указывается на шаге 5 мастера.
...
3. Скопируйте **token**
4. В `install.sh` на шаге **3a** выберите DuckDNS, введите поддомен и token
5. На шаге **5** выберите Let's Encrypt, домен подставится автоматически
...
3. В `install.sh` на шаге **3a** выберите No-IP, введите hostname, логин и пароль
4. Не забудьте подтверждать hostname каждые 30 дней (бесплатный план)

### Свой домен / Cloudflare / deSEC

Купите домен у регистратора, делегируйте DNS на Cloudflare или deSEC, создайте A-запись на публичный IP сервера. В мастере на шаге **3a** выберите «Не использую DDNS», на шаге **5** — Let's Encrypt и укажите свой домен.
