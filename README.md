...
- ограничите доступ по **белому списку IP**, если знаете свои адреса;
- ротация `SECRET_KEY`, API-ключей и Telegram token — **Настройки → Безопасность** (wizard с подтверждением).
